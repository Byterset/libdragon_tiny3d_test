...
/// @brief GJK Support function (see gjk_support_function) for the cylinder primitive.
///
/// Will return the point on the cylinder boundary furthest in the input direction.
/// @param data pointer to the physics_object holding the cylinder collider
...
/// @brief Bounding Box Calculator function for the cylinder primitive.
///
/// Calculates the AABB fully containing the (optionally rotated) cylinder.
/// @param data pointer to the physics_object holding the cylinder collider
/// @param rotation pointer to the physics_object rotation Quaternion (may be NULL)
/// @param box  pointer to the output AABB
void cylinder_bounding_box(const void* data, const Quaternion* rotation, AABB* box);

/// @brief Inertia Tensor function for the cylinder primitive.
///
