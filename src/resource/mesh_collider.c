...
void mesh_collider_load_test(struct mesh_collider* into){
    ...
    AABBTree_create(&into->aabbtree, 4);
    struct AABB triangleAABB;
    for (int i = 0; i < triangle_count; i++)
    {
        struct mesh_triangle_indices* triangle = &into->triangles[i];
        struct Vector3* v0 = &into->vertices[triangle->indices[0]];
        struct Vector3* v1 = &into->vertices[triangle->indices[1]];
        struct Vector3* v2 = &into->vertices[triangle->indices[2]];
